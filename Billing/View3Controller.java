    ...
    void doGetDays(ActionEvent event) 
    {
    	try{
    		...
    		LocalDate ldobe = lbldoe.getValue();
    		 days = (int)ChronoUnit.DAYS.between(ldobs, ldobe);
    		 ddays.setText(days+"");
    				...
    				String name2=lblList2.getSelectionModel().getSelectedItem();
    				 ...
    				 try {
    						...
    						while(table.next())
    						{
    							...
    							CQ.setText(String.valueOf(total));     // CoolText
    							
    							cbill = (total*Float.parseFloat(Cbill.getText())) *days;
    							bbill = (total2*Float.parseFloat(Bbill.getText())) *days;
    							fbill = cbill+bbill;
